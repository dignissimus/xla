...
TEST_F(YnnReduceTest, SliceSubtractSquareReduce) {
  ...
  ENTRY main {
    ...
    slice_a = f32[64,63] slice(input), slice={[0:64], [1:64]}
    slice_b = f32[64,63] slice(input), slice={[0:64], [0:63]}
    diff = f32[64,63] subtract(slice_a, slice_b)
    squared = f32[64,63] multiply(diff, diff)
    init = f32[] constant(0)
    ROOT result = f32[64] reduce(squared, init), dimensions={1}, to_apply=add
  }
  )";
